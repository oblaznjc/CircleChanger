...
class CircleChanger(object):
    ...
    def swallow(self, other_circle_changer):
        ...
        new_radius = self.center.get_distance_from(other_circle_changer.center) / 2
        new_circle = rg.Circle(new_center, new_radius)
        new_colors = self.colors + other_circle_changer.colors
        new_circle_changer = CircleChanger(new_center.x, new_center.y, new_radius, 'red', new_colors)
    ...
    def change_to_next_color_in_tuple(self):
        ...
        self.count = self.count % len(self.colors) + 1
